...
async def read_page(
    ...
    retry = 0
    while True:
        raw, status = await aio_get(APIS[1].format(bid=bid, cid=cid, page=page))
        if status != 200:
            retry += 1
        else:
            break
    ...
    content: list[Tag] = soup.find("div", {"id": "TextContent"}).find_all("p")[:-1]
    ...
    return [
        str(i).replace("<p>", "<p>&nbsp;&nbsp;").replace("[email&#160;protected]", "@")
        for i in content
...
async def read_chapter(
    ...
    retry = 0
    while True:
        raw, status = await aio_get(APIS[1].format(bid=bid, cid=cid, page=2))
        if status != 200:
            retry += 1
        else:
            break
...
async def download_book(title: str, chapters: dict[str, str]) -> dict[str, list[str]]:
...
async def get_toc(
    ...
    raw, _ = await aio_get(APIS[0].format(bid=bid))
    soup = Soup(raw.decode("gbk"), features="html.parser")

    title = soup.find("div", {"class": "d_title"}).find("h1").text
    author = soup.find("div", {"id": "count"}).find("a").text
    toc: list[Tag] = soup.find("ul", {"id": "chapterList"}).find_all("a")
...
async def download(bid: str) -> tuple[str, str, dict[str, dict[str, list[str]]]]:
    ...
    all_episode_content = await asyncio.gather(
        *[download_book(title, ep) for title, ep in episodes.items()]
    )
